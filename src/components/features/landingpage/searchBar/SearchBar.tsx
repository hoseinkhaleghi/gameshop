...
import { DataContext, DataContextType, Game } from "../../../../GlobalStates/DataProvider";
...
function SearchBar() {
  ...
  const [finalinput, setFinalinput] = useState<Game[]>([]); // تغییر نوع به Game[]
  ...
  return (
    <section className={isDarkMode ? "text-white bg-gray-800 lg:flex flex-row rounded-md m-5" : "text-black bg-stone-50 border-2 lg:flex flex-row rounded-md m-5"}>
      <div className={i18n.language === "en" ? (isDarkMode ? "text-white bg-gray-900 p-10 lg:flex flex-col rounded-br-full gap-4 justify-center items-center rounded-md hidden" : "text-black bg-stone-200 p-10 lg:flex flex-col rounded-br-full gap-4 justify-center items-center rounded-md hidden") : (isDarkMode ? "text-white bg-gray-900 p-10 lg:flex flex-col rounded-bl-full gap-4 justify-center items-center rounded-md hidden" : "text-black bg-stone-200 p-10 lg:flex flex-col rounded-bl-full gap-4 justify-center items-center rounded-md hidden")}>
        <p>{t("forSearch.messsage")}</p>
        <div className="text-red-400 text-5xl"><MdOutlineContentPasteSearch /></div>
      </div>
      <div className="w-11/12 flex flex-col p-10">
        <SearchInput setFinalinput={setFinalinput as React.Dispatch<React.SetStateAction<Game[]>>} />
        <div className="flex flex-col md:flex-row w-full gap-4 md:gap-10 mt-6">
          <label htmlFor="company">{t("Developer.messsage")}</label>
          <select onChange={(e) => setFinalcompany(e.target.value)} name="company" id="company" value={finalcompany} className="w-72 text-black">
            {Company.map((item, index) => (
              <option key={index}>{item}</option>
            ))}
          </select>
          <label htmlFor="genre">{t("Genre.messsage")}</label>
          <select onChange={(e) => setFinalgenre(e.target.value)} value={finalgenre} name="genre" id="genre" className="w-72 text-black">
